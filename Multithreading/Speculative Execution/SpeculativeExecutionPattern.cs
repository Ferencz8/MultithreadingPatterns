using Multithreading.Patterns.Speculative_Execution;
using System;
...
namespace Multithreading.ConsoleUI.Speculative_Execution
{
    ///Here i just copy and pasted an example from Pluralsight.
    /// <summary>
    /// The speculative execution pattern
    /// You create a set of tasks and the first one to finish wins. You harvest their value and cancel the rest.
    /// 
    /// Use case: when you have multiple sources for generating a result(e.g. calling web services/back web service). 
    /// The idea is you fire up multiple tasks each requesting data from a source and the first task to finish wins and cancels the rest
    /// </summary>
    public class SpeculativeExecutionPattern
    {
        static SpeculativeExecutionPattern()
        {
            // 
